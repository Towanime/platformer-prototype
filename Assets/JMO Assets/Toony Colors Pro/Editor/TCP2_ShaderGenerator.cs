...
public class TCP2_ShaderGenerator : EditorWindow
{
	...
	static private string TemplateInfo = null;
	static private string TemplateWarning = null;
	static private string TemplateType = null;

	static private TextAsset TryLoadTemplate(string configType = null)
	{
		if (!string.IsNullOrEmpty(configType))
		{
			switch (configType)
			{
				case "terrain": return LoadTextAsset("TCP2_User_Unity5_Terrain.txt");
			}
		}

		return LoadTextAsset("TCP2_User_Unity5.txt");
	}

	static private TextAsset LoadTextAsset(string filename)
	{
	...
	void OnGUI()
	{
		...
		if(!string.IsNullOrEmpty(TemplateWarning))
		{
			EditorGUILayout.HelpBox(TemplateWarning, MessageType.Warning);
		}
		...
		if(GUILayout.Button("Re-Generate All", GUILayout.Width(120f), GUILayout.Height(30f)))
		{
			...
			foreach(Shader s in mUserShaders)
			{
				...
				{
					TCP2_ShaderGeneratorUtils.Compile(mCurrentConfig, s, Template.text, false, !mOverwriteConfigs, mIsModified);
				}
			}
		...
		if(GUILayout.Button(mCurrentShader == null ? "Generate Shader" : "Update Shader", GUILayout.Width(120f), GUILayout.Height(30f)))
		{
			...
			{
				mCurrentConfig.configType = TemplateType;
			}

			Shader generatedShader = TCP2_ShaderGeneratorUtils.Compile(mCurrentConfig, mCurrentShader, Template.text, true, !mOverwriteConfigs, mIsModified);
			ReloadUserShaders();
		...
		EditorGUILayout.BeginHorizontal();
		TCP2_ShaderGeneratorUtils.CustomOutputDir = GUILayout.Toggle(TCP2_ShaderGeneratorUtils.CustomOutputDir, new GUIContent("Custom Output Director:", "Will save the generated shaders in a custom directory within the Project"), GUILayout.Width(160f));
	...
	private void FeaturesGUI()
	{
		...
		//Color Multipliers
		GUISingleFeature("COLOR_MULTIPLIERS", "Colors Multipliers", "Adds multiplier values for highlight and shadow colors to enhance contrast or colors", showHelp:false, indentSpacing:24f);
		//Textured Threshold
		...
		GUISingleFeature("DISABLE_WRAPPED_LIGHTING", "Disable Wrapped Lighting", "Disable wrapped diffuse lighting, making lights appear more focused");
		Space();
		//----------------------------------------------------------------
		//Detail
		...
		//----------------------------------------------------------------
		//Self-Illumination
		GUIMask("Emission Map", null, "ILLUMIN_MASK", "ILLUMIN_MASK_CHANNEL", "ILLUMINATION", helpTopic:"Self-Illumination Map");
		//Self-Illumination Color
		GUISingleFeature("ILLUM_COLOR", "Emission Color", null, HasFeat("ILLUMINATION"), true);
		Space();
		//----------------------------------------------------------------
		//Bump
		GUISingleFeature("BUMP", "Normal/Bump map", helpTopic: "normal_bump_map_sg");
		//BumpScale
		GUISingleFeature("BUMP_SCALE", "Bump Scale", null, HasFeat("BUMP"), true);
		//Parallax
...
//		Space();
		...
		GUISingleFeature("SUBSURFACE_SCATTERING", "Subsurface Scattering");
		GUIMultipleFeaturesInternal("Subsurface Lights", "Defines which lights will affect subsurface scattering",
									new string[] { "Point\\Spot Lights|", "Directional Lights|SS_DIR_LIGHTS", "All Lights|SS_ALL_LIGHTS" },
									HasFeat("SUBSURFACE_SCATTERING"), true, null, false, 166);
		GUIMask("Subsurface Mask", "Defines where the subsurface scattering effect should apply", "SS_MASK", "SS_MASK_CHANNEL", "SS_MASK", HasFeat("SUBSURFACE_SCATTERING"), true);
		GUISingleFeature("SS_ADDITIVE", "Additive", "Makes the subsurface scattering effect added to the diffuse color instead of multiplied with it", HasFeat("SUBSURFACE_SCATTERING"), true, showHelp:false);
		Space();
		//----------------------------------------------------------------
		//Cubemap Ambient
		...
		//Rim Mask
		GUISingleFeature("RIM_LIGHTMASK", "Light-based Mask", "Will make rim be influenced by nearby lights", HasFeat("RIM"), true);
		Space();
		//----------------------------------------------------------------
		//MatCap
	...
	private void FeaturesGUI_Terrain()
	{
		...
		//----------------------------------------------------------------
		//Specular
		GUISingleFeature("SPECULAR", "Specular", null);
		//GUIMultipleFeaturesHelp("Specular", null, "specular_sg", "Off|", "Regular|SPECULAR", "Anisotropic|SPECULAR_ANISOTROPIC");
	...
	private void GUISingleFeature(string featureName, string label, string tooltip = null,
	                              ...
	                              string helpTopic = null, bool showHelp = true, float indentSpacing = 0f)
	{
		if(!enabled)
			GUI.enabled = false;
	...
	static private void UpdateTemplateMeta()
	{
		TemplateInfo = null;
		...
		{
			using (System.IO.StringReader reader = new StringReader(Template.text))
			{
				string line;
	...
	private void LoadCurrentConfig(TCP2_Config config)
	{
		...
		Template = TryLoadTemplate(mCurrentConfig.configType);
	}
	...
	{
		string rootPath = Application.dataPath + (mLoadAllShaders ? "" : TCP2_ShaderGeneratorUtils.OutputPath);
